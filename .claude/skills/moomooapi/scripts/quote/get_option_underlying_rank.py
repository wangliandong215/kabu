...
def _resolve_sort_type(sort_str):
    from moomoo import UnderlyingRankSortType
...
def _build_filters(spec):
    if not spec or not spec.get("filters"):
        return None
    from moomoo import UnderlyingRankIndicatorType, StockCategory
    from moomoo.quote.quote_option_event_info import UnderlyingRankFilter
