...
def _build_alert_item(spec, key=None):
    import moomoo
    from moomoo import OptionEventAlertItem
    ...
    if "option_market" in spec:
        kwargs["option_market"] = _resolve_enum(moomoo, "OptionMarket", spec["option_market"])
    ...
    if "option_type" in spec:
        kwargs["option_type"] = _resolve_enum(moomoo, "OptionType", spec["option_type"])
    if "side_type_list" in spec:
        kwargs["side_type_list"] = [_resolve_enum(moomoo, "EventTickerType", v) for v in spec["side_type_list"]]
    if "order_type_list" in spec:
        kwargs["order_type_list"] = [_resolve_enum(moomoo, "AlertOrderType", v) for v in spec["order_type_list"]]
...
def set_option_event_alert(op, key=None, config_path=None, output_json=False):
    import moomoo
    from moomoo import AlertOpType

    op_type = _resolve_enum(moomoo, "AlertOpType", op)
