...
def _resolve_sort_type(sort_str):
    from moomoo import OptionRankType
...
def _build_filters(spec):
    if not spec or not spec.get("filters"):
        return None
    from moomoo import OptionRankIndicatorType, StockCategory
    from moomoo.quote.quote_option_event_info import OptionRankFilter
