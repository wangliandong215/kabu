...
def _resolve_option_market(market_str):
    from moomoo import OptionMarket
...
def _build_filters(spec):
    if not spec or not spec.get("filters"):
        return None
    from moomoo import EventIndicatorType
    from moomoo.quote.quote_option_event_info import OptionEventFilter
...
def _build_sort(spec):
    if not spec or not spec.get("sort"):
        return None
    from moomoo import EventIndicatorType, EventSortDir
    from moomoo.quote.quote_option_event_info import OptionEventSort
