...
def _resolve_sort_type(sort_str):
    if not sort_str:
        return None
    from moomoo import SellerSortType
...
def _build_filters(spec):
    if not spec or not spec.get("filters"):
        return None
    from moomoo import SellerIndicatorType, StockCategory
    from moomoo.quote.quote_option_event_info import SellerFilter
