...
### Prediction Market (13 queries + 3 subscriptions + 3 push Handlers)
...
```
get_event_contract_category(category=None)  -- Prediction market category list
...
get_event_contract_snapshot(code_list)  -- Prediction market snapshot (no subscription required)
get_event_contract_order_book(code, num=10)  -- Prediction market order book (requires ORDER_BOOK subscription; returns dict{yes_bids/asks, no_bids/asks})
get_event_contract_kline(code, pre_side=None, ktype=KLType.K_DAY, kline_source=None, max_count=1000)  -- Prediction market K-line (requires corresponding K-line subscription)
get_event_contract_ticker(code, count=30)  -- Prediction market ticker (requires TICKER subscription)
request_history_event_contract_kline(code, start=None, end=None, pre_side=None, ktype=KLType.K_DAY, kline_source=None, max_count=1000, page_req_key=None)  -- Prediction market historical K-line (no subscription needed; auto-paginated)
subscribe_event_contract(code_list, subtype_list, kline_source_list=None, is_first_push=True, subscribe_push=True)  -- Subscribe prediction market
unsubscribe_event_contract(code_list, subtype_list, kline_source_list=None)  -- Unsubscribe (by dimension)
unsubscribe_all_event_contract()  -- Unsubscribe all prediction markets on the current connection
```
...
EventContractOrderBookHandlerBase  -- Prediction market order book push callback (requires subscribe_event_contract ORDER_BOOK)
EventContractKlineHandlerBase      -- Prediction market K-line push callback (requires corresponding K-line subscription)
EventContractTickerHandlerBase     -- Prediction market ticker push callback (requires TICKER subscription)
```
